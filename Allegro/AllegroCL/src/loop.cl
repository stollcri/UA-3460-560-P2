#+(version= 8 0)
(sys:defpatch "loop" 1
  "v1: Fix type propagation of for-across and do-repeat loops."
  :type :system
  :post-loadable t)

;;;   -*- Mode: LISP; Syntax: Common-lisp; Base: 10; Lowercase:T -*-
;;;
;;; copyright (c) 1989-2007 Franz Inc, Oakland, CA - All rights reserved.
;;;>
...
;;;>      +1-617-253-1000
;;;>
;;;> Copyright (c) 1989, 1990, 1991, 1992 by Symbolics, Inc.
;;;> All Rights Reserved.
;;;> 
;;;> Permission to use, copy, modify and distribute this software and its
;;;> documentation for any purpose and without fee is hereby granted,
...
;; $Id: loop.cl,v 1.37 2008/12/15 19:55:27 duane Exp $
...
(autoload-from "code/misc.cl"
	       complex-loop-expander)

;; [bug13189]: Temporary for 6.2: DO NOT JOIN w/7.0!
(when (boundp '*modules*)
  (setq *modules* (delete (string :loop2) *modules* :test #'equal)))
...
;; spr30884 analysis: If there are multiple maximize/minimize clauses, then
;; this macro creates nested shadowing lets.  It would be best to figure out
;; how to eliminate creating these lets, but adding ignorable declarations
;; allows compilation without warnings.
;; cac 23dec05
...
(defun print-loop-universe (u stream level)
  ...
  (let ((str (case (loop-universe-ansi u)
    ...
    #+(or Genera cmu allegro)		;@@@@ This is reallly the ANSI definition.
    (print-unreadable-object (u stream :type t :identity t)
      (princ str stream))
    #-(or Genera CLOE cmu allegro)
...
#-allegro
(defmacro symbol-name-by-mode (sym mode)
  (declare (ignore mode))
  `(symbol-name ,sym))

#+allegro
(defmacro symbol-name-by-mode (symbol mode)
  (let ((sym (gensym)))
    `(let ((,sym ,symbol))
       (if ,mode
	   (symbol-name-other-case ,sym)
	 (symbol-name ,sym)))))

(defun make-standard-loop-universe (&key keywords for-keywords iteration-keywords path-keywords
				    type-keywords type-symbols ansi mode)
  ...
  (flet ((maketable (entries)
	   (let* ((size (length entries))
	     ...
	     (dolist (x entries) (setf (gethash (symbol-name-by-mode (car x) mode) ht) (cadr x)))
	     ht)))
    (make-loop-universe
...
(defun loop-constant-fold-if-possible (form &optional expected-type)
  ...
  (let ((new-form form) (constantp nil) (constant-value nil)
    ...
    #+allegro (setq constantp (constantp new-form env)
		    constant-value (and constantp (sys:constant-value new-form env)))
    #-(or Genera allegro)
...
(defun loop-constantp (form)
  #+(or Genera allegro) (constantp form *loop-macro-environment*)
  #-(or Genera allegro) (constantp form))

...
(setf (get 'end-loop 'ignorable)  t	;bug2276 7jul94 smh
      (get 'next-loop 'ignorable) t)

(defun loop-body1 (env
		   prologue
		   before-loop
		   main-body
		   after-loop
  ...
  (unless (= (length before-loop) (length after-loop))
    (excl::.error "LOOP-BODY called with non-synched before- and after-loop lists."))
  ...
  (labels ((psimp (l)
    ...
    (when (or *loop-duplicate-code* (not rbefore))
      (return-from loop-body1 (makebody)))
	...
	(pop rafter))
	  ...
	  ((null bb) (return-from loop-body1 (makebody))) ;Did it.
...
(defun estimate-code-size-1 (x env)
  (flet ((list-size (l)
    ...
    (cond ((constantp x #+(or Genera allegro) env) 1)
	       ...
	       (cond ((setq tem (get fn 'estimate-code-size))
			...
			(setq n (+ n (estimate-code-size-1 (cadr l) env) 1))))
		     ...
		     ((eq fn 'function)
		      ...
		      (if (function-name-p (cadr x))
			  1
			  (throw 'estimate-code-size nil))) ; [bug11294]
...
(defun loop-error (format-string &rest format-args)
  ...
  #+allegro (excl::.program-error "~?~%Current LOOP context:~{ ~S~}."
				  format-string format-args (loop-context))
  #+cmu (kernel:simple-program-error "~?~%Current LOOP context:~{ ~S~}."
				     format-string format-args (loop-context)))
...
(defun loop-translate (*loop-source-code* *loop-macro-environment* *loop-universe*)
  (let ((*loop-original-source-code* *loop-source-code*)
    ...
    (loop-bind-block)
    (let ((answer (loop-body1 *loop-macro-environment*
...
(defun loop-do-if (for negatep)
  (let ((form (loop-get-form))
    ...
    ;; Next clause moved outside *loop-inside-conditional* binding - bug3376 smh 7oct94
    (loop-pseudo-body
  ...
  (let* ((form (loop-get-form))
	 ...
	 (type
	  ...
	  (if (constantp form env)
	      (type-of (sys:constant-value form env))
	    'real)
	  #+allegro
	  (with-normal-consing
	      (if (constantp form env)
		  (type-of (sys:constant-value form env))
		;; [rfe6664]: We can look into the environment for a more specific
		;; type
		(if (symbolp form)
		    (multiple-value-bind (kind valcell type)
			(sys:variable-information form *loop-macro-environment*)
...
(defun loop-for-across (var val data-type)
  ...
  (let ((vector-var (loop-gentemp 'loop-across-vector-))
	...
	(if (and (consp vector-form) (eq (car vector-form) 'the))
	  ...
	  ;; [rfe6664]: We can look into the environment for a more specific
	  ;; type
	  (if (symbolp vector-form)
	      (multiple-value-bind (kind valcell type)
		  (sys:variable-information vector-form *loop-macro-environment*)
		(declare (ignore valcell))
		(unless (and (eq kind :lexical)
			     type
			     (setq type (assoc 'type type))
...
(defun add-loop-path (names function universe &key preposition-groups inclusive-permitted user-data mode)
  ...
  (let ((ht (loop-universe-path-keywords universe))
	(lp (make-loop-path
	      :names (mapcar #'(lambda (name) (symbol-name-by-mode name mode)) names)
	      :function function
	      :user-data user-data
	      :preposition-groups (mapcar #'(lambda (x) (if (listp x) x (list x))) preposition-groups)
	      :inclusive-permitted inclusive-permitted)))
    (dolist (name names) (setf (gethash (symbol-name-by-mode name mode) ht) lp))
    lp))

...
(defun loop-for-arithmetic (var val data-type kwd)
  (loop-sequencer
    var (loop-check-data-type data-type *loop-real-data-type* #+allegro nil) t
  ...
  (let ((pkg-var (loop-gentemp 'loop-pkgsym-))
    ...
    `(((,variable nil ,data-type) (,pkg-var ,pkg))
      ...
      (not (multiple-value-setq (,(progn
				 ...
				 ,variable)
	     (progn ,(loop-when-it-variable) ; ignore [bug13764]
		    (,next-fn))))
      ())))

;;;; ANSI Loop

(defun make-ansi-loop-universe (extended-p mode)
...
(defparameter *loop-ansi-universe*
    (make-ansi-loop-universe nil
			     #-allegro t
			     #+allegro
			     (eq *current-case-mode-number* case-mode-sensitive-lower)))


#+allegro
(defparameter *loop-local-universe*
    (make-ansi-loop-universe t (not (eq *current-case-mode-number* case-mode-sensitive-lower))))


#+ignore
(defun loop-rebuild-ansi-universe ()
  (setq *loop-ansi-universe* (make-ansi-loop-universe nil nil)))

;; if the case of symbols is switched we have to rebuild our tables
#+ignore
(pushnew #'loop-rebuild-ansi-universe excl::*symbol-case-convert-hook*)
...
(defmacro loop (&environment env &rest keywords-and-forms)
  #+Genera (declare (compiler:do-not-record-macroexpansions)
		    (zwei:indentation . zwei:indent-loop))
  (loop-standard-expansion keywords-and-forms env
			   (if (eq excl::*case-translation* :local)
			       *loop-local-universe*
			     *loop-ansi-universe*)))

#+allegro
(defun excl::complex-loop-expander (body env)
  (loop-standard-expansion body env
			   (if (eq excl::*case-translation* :local)
			       *loop-local-universe*
			     *loop-ansi-universe*)))
