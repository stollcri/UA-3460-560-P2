...
(defvar *checklinks-version* "1.46")
...
(defun main ()
  (flet
	 ...
	 (system:with-command-line-arguments
	     ("2A:ac:Dh:i:lo:Or:vx:X:Y:Z:"
	      pass2-ignore-non-html allow-hosts check-all cgi-bin
	      debug-mode ignore-hosts icons-dir local-only
	      orphans-reference-file ignore-orphans doc-root verbose
	      ignore-files ignore-directories pass2-ignore-directories
	      debug-on-uri)
	     (files :usage *usage*)
	   ...
	   (format t "~&~%Total links checked: ~D.~%" *link-counter*)
	   (exit .status. :quiet t))))
	  ...
	  (let ((href (second
		...
		(when uri
		  ...
		  (if* (or *client-mode* (uri-scheme unmerged-uri))
		     then (and (eq :all *verbose*) (format t "  push on checklist~%"))
			  (push (list position uri (list myuri position))
				checklist)
		     else (and (eq :all *verbose*) (format t "  check-filespec~%"))
...
(defun check-remote-uri-1 (uri &optional (method :head))
  (flet ((doit ()
	   (multiple-value-bind (body code headers)
	    (mp:with-timeout (*timeout* :timed-out)
	      (if* (eq :head method)
