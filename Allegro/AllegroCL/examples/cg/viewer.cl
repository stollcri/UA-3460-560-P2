  ...
  (unless directory
    (setq directory
          (ask-user-for-directory
           :stream window
           :prompt
           "Select a directory with .bmp pixmap files in it.")))
  (unless directory
    ...
    (setf (reverse-pixmap-paths window)
      (reverse
       (setf (pixmap-paths window)
         (mapcar #'(lambda (path)
                     (list path (load-texture-info path)))
           files))))
...
(defun pixmap-viewer-view-file (window path)
  (unless path
    ...
    (window-message
        window
        "There are no .bmp pixmap files in this directory.")
