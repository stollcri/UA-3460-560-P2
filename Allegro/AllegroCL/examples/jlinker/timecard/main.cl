...
;; $Id: main.cl,v 5.5 2005/10/12 01:25:02 mm Exp $
...
(defun query-awt (&optional restart &aux v)
  (if* (jlinker-query)
       ...
       elseif (consp (setf v (jlinker-init nil :verbose t)))
...
(defun main-top () 
  ...
  (time (jlinker-init nil :preload nil))
  (jlookup :new)
...
(defmethod init-main ((self main-panel) &key 
		      timetable state
		      &allow-other-keys)
  (setf (main-connection self) (jlinker-slot :server))
...
(defmethod main-alive-p ((main main-panel))
  (with-slots 
   (connection) main 
   (and connection (eq connection (jlinker-slot :server)))))
    ...
    (case how
	 ...
	 (format nil "~A ~A" mo dy)))
      (otherwise
	   ...
	   (pretty-date year month day)
	 (format nil "~A, ~A ~A ~A ~A" wk mo dy yr))))))
