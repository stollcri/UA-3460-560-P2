...
(defun make-socket-server
  (&key
    (name (format nil "socket server ~d " (incf *server-count*)))
    port 
    function 
    wait 
    (format :text)
...
(defun start-socket-server (passive-socket &key function wait)
  ...
  (unwind-protect
        ...
        (if wait
          (progn 
            (unwind-protect
              ...
              (handler-case
                (values-list 
                  (cons t (multiple-value-list (close connection)))
                )
                (error
                  (condition)
    ...
    (handler-case
      (values-list
        (cons t
          (multiple-value-list (close passive-socket))
        )
      )
      (error
        (condition)
...
(defun process-apache-command (command)
  (let
    ((html
      (if
        (equal
          (cdr (assoc "url" command :test #'string=)) 
          "/modlisp/fixed"
        )
        (debug-table command)
        (fixed-html)
...
(defun fixed-html ()
  ...
  <h1>mod_lisp 2.0</h1>
  <p>This is a constant html string sent by mod_lisp 2.0 + CLISP + apache + Linux</p>
  </body></html>"
)
